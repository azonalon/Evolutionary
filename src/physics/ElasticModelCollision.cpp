...
double strengthSelfCollision = 100;
                                                     ...
                                                     VectorD &dest)
{
    ...
    {
        E += addSelfCollisionPenaltyGradient(triplet, MI, x, dt, dest, v);
    }
    return E;
};
...
void ElasticModel::populateSelfCollisionList(const VectorD &x)
{
    ...
    for (unsigned l = 0; l < m; l++)
    {
        ...
        for (unsigned u = 0; u < x0.size() / 2; u++)
        {
            ...
            temp2x2A << x[i + 0] - x[k + 0], x[j + 0] - x[k + 0],
      ...
      */
            ...
            {
                colliding[iPoint] = 1.0;
                colliding[iPoint + 1] = 1.0;
    ...
    for (auto it = ei.first; it != ei.second; it++)
    {
        ...
        if (!g[chain].visited)
        {
            ...
            if (surface.size() > 1)
            {
                ...
                brains.push_back(NeuralNetwork(n, surface.size()));
            }
        }
    }
